const articleContents = {
    "ai-web-development": {
        ...
        content: `
            ...
            <p><a href="https://www.w3.org/standards/webdesign/artificial-intelligence" target="_blank" style="color: #2563eb; text-decoration: none;">Read more about AI in web development</a></p>
    ...
    "javascript-frameworks": {
        ...
        content: `
            ...
            <p><a href="https://react.dev/" target="_blank" style="color: #2563eb; text-decoration: none;">Learn more about React</a></p>
            <p><a href="https://vuejs.org/" target="_blank" style="color: #2563eb; text-decoration: none;">Learn more about Vue.js</a></p>
            <p><a href="https://angular.dev/" target="_blank" style="color: #2563eb; text-decoration: none;">Learn more about Angular</a></p>
...
.optimized-animation {
    ...
    will-change: transform; /* Hint to browser */
            ...
            <p><a href="https://developer.mozilla.org/en-US/docs/Web/CSS/CSS_Animations" target="_blank" style="color: #2563eb; text-decoration: none;">Explore CSS Animations on MDN</a></p>
...
app.use(errorHandler);
            ...
            <p><a href="https://nodejs.org/en/docs/" target="_blank" style="color: #2563eb; text-decoration: none;">Explore Node.js Documentation</a></p>
...
class MyWidget extends StatelessWidget {
    ...
    Widget build(BuildContext context) {
        return Scaffold(
            ...
            <p><a href="https://reactnative.dev/" target="_blank" style="color: #2563eb; text-decoration: none;">Explore React Native</a></p>
            <p><a href="https://flutter.dev/" target="_blank" style="color: #2563eb; text-decoration: none;">Explore Flutter</a></p>
...
const topPrediction = predictions.indexOf(Math.max(...predictions));
            ...
            <p><a href="https://www.tensorflow.org/js" target="_blank" style="color: #2563eb; text-decoration: none;">Get Started with TensorFlow.js</a></p>
...
function showArticleModal(title, category, excerpt, date, readTime, articleKey) {
    try {
        ...
        const articleData = articleContents[articleKey] || {
            ...
            content: `<p>${sanitizeHTML(excerpt)}</p><p>Full article content would go here...</p>`
        };
        
        modalContent.innerHTML = `
            <span class="close">×</span>
            <h2>${sanitizeHTML(articleData.title)}</h2>
            <div style="margin: 1rem 0; color: #666;">
                <span style="background: #667eea; color: white; padding: 0.25rem 0.5rem; border-radius: 4px; font-size: 0.875rem;">${sanitizeHTML(articleData.category)}</span>
                <span style="margin-left: 1rem;">${sanitizeHTML(date)}</span>
                <span style="margin-left: 1rem;">${sanitizeHTML(readTime)}</span>
            </div>
        ...
        alert('Failed to load article content. Please try again.');
    }
}
...
// Function to initialize article cards with click events
function initializeArticleCards() {
    ...
    articleCards.forEach(card => {
        card.addEventListener('click', () => {
            ...
            const date = card.querySelector('.article-meta .date').textContent;
            const readTime = card.querySelector('.article-meta .read-time').textContent;
            const articleKey = getArticleKey(title);
            showArticleModal(title, category, excerpt, date, readTime, articleKey);
        });
    });
}
...
function filterArticles(category) {
    ...
    articleCards.forEach(card => {
        ...
        if (category === 'all' || cardCategory.toLowerCase() === mappedCategory.toLowerCase()) {
...
function initializeLoadMore() {
    ...
    // Initially show only first few articles
    allArticles.forEach((card, index) => {
        card.style.display = index < articlesPerPage ? 'block' : 'none';
    });

    if (loadMoreBtn) {
        loadMoreBtn.addEventListener('click', () => {
            const nextArticles = allArticles.slice(visibleArticles, visibleArticles + articlesPerPage);
            nextArticles.forEach(card => {
                card.style.display = 'block';
                card.classList.add('fade-in'); // Add animation for new articles
            });
...
function initializeTagFiltering() {
    ...
    tags.forEach(tag => {
        tag.addEventListener('click', () => {
            const category = tag.getAttribute('data-category').toLowerCase();
    ...
    const allTag = Array.from(tags).find(tag => tag.getAttribute('data-category').toLowerCase() === 'all');
    if (allTag) {
...
document.addEventListener('DOMContentLoaded', () => {
    try {
        ...
        console.error('Error initializing article features:', error);
    }
});

// Animation for newly loaded articles
